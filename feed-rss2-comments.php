	...
	<item>
		<title><?php
			if ( !is_singular() ) {
				...
				printf(ent2ncr(__('Comment on %1$s by %2$s')), $title, get_user_meta( $comment->user_id, 'nickname', true ));
			} else {
				//printf(ent2ncr(__('By: %s')), get_comment_author_rss());
				printf(ent2ncr(__('By: %s')), get_user_meta( $comment->user_id, 'nickname', true ));
			}
		?></title>
